...
- CONNECT: candidate is a new fact that relates to ONE specific existing fact (named by old_fact_id). Insert the new fact AND record a connection from new → old. Choose the predicate that best describes the relationship:
    ...
    expands       — same fact, more specific. Examples: "Anna has a pet" → "Anna has a dog named Nico"; "lives in Italy" → "lives in Rome".
...
New candidates:
  ...
  [3] (event, conf=0.90) Anna's family dog passed away yesterday.

Correct ops:
  - CONNECT: candidate_index=0, old_fact_id=7, predicate=update — Anna moved Berlin → Lisbon. The new fact replaces the location claim; old fact stays in memory as the prior state.
  - DROP: candidate_index=1 — fact 3 already covers Anna's jazz preference with more detail.
...
- Use stable local_id values like "subject", "venice", "giulia"; local_id only needs to be unique within this response.

...
**Is.** A *per-subject* memory store. Every fact has a subject (`me`, `emma venezia`, `mom`, …). Retrieval ranks facts by relevance to a query *and* by which subjects the query mentions. The engine is source-agnostic: anything that can produce a `(contact, message, ts, direction)` tuple can feed it.
